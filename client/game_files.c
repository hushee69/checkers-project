...
char *tab_pseudo[MAX_CLIENT]; // tableau qui va contenire 10 pseudo
char *tab_motDepasse[MAX_CLIENT];
int nbJoueurActuelle = 0; // c'est le nombre de joueur actuelle dans la base de donnee pas dans le jeu
...
void ajouteJoueur( char pseudo[], char motpasse[], char* tab_pseudo[], char* tab_motDepasse[], int *nbJoueur){
	...
	FILE *flux = ouvrireFichier("test.txt");
...
void recupereJoueurs( FILE *flux, char *tab_pseudo[], char *tab_motDepasse[], int *nbJoueur){
	...
	printf("nb joueur actuelle est %d\n", i);
	free(tab);
